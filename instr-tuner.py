...
def futhark_bench_cmd_experimental(cfg, json, times, tile, logfile):
    def sizeOption(size):
        return '--pass-option --size={}={}'.format(size, cfg[size])
    size_options = ' '.join(map(sizeOption, cfg.keys()))
    cmd = 'futhark bench --skip-compilation --exclude-case=notune {} '.format(program)
    if json != None:
        cmd += '--json={} '.format(json.name)
    if logfile != None:
        cmd += '--pass-option=-D --json={} '.format(logfile.name)
    if times != None:
        if not (any(np.isinf(val) for val in times.values())):
            cmd += '--timeout={} '.format(compute_timeout(times))
    if tile != None:
...
for program in programs:
    ...
    #======================#=================================================================================#
    # The goal of the Experimental Instrumentation Tuner is to use additional information to solve the task. #
    # This additional information is to break down the loop-based runtimes into their intermediates for each #
    # iteration, and to use those as the primary driver. This fundamentally changes the approach, and allows # 
    # much faster and more optimal results to be generated.                                                  #
    #========================================================================================================#
    ...
    if len(conflicts) != 0:
        ...
        comp_re    = re.compile('Compared+ ([^ ]*) <= (\d*)')
        runtime_re = re.compile('kernel+ ([^ ]*) runtime: (\d*)u')
        ...
        for depth, i in deepest_first_order:
            ...
            for j, version in enumerate(branch_versions):
                # First, find the deepest True threshold, indicating the current tuning threshold
                position = -1
                ...
                if len(loop_options) > 2:
                    print("[{}s] Tuning threshold {} in the loop-based setting using instrumentation".format(int(time.time() - start), name))
                    
                    # RUN THE FALSE VERSION                
                    iterF = []
                    ...
                    with tempfile.NamedTemporaryFile() as json_tmp: 
                        if path in execution_cache:
                            ...
                            bench_cmd = futhark_bench_cmd_experimental(conf, None, None, 16, json_tmp)
                            ...
                            results = json_data[program]['datasets']
                            execution_cache[path] = results
                            
                        for dataset in results.keys():
                            iter = []
                            stderr = results[dataset]['stderr'].split('\n')
                            
                            for i, line in enumerate(stderr):    
                                if name in line: #We have a line referencing the threshold we look for.
                                    regex = comp_re.search(line)
                                    t = regex.group(1)
                                    e_par = regex.group(2)
                                    
                                    runtime = 0
                                    for new_line in stderr[i+1:]:
                                        if name in new_line:
                                            break
                                        if "kernel " in new_line and " runtime: " in new_line:
                                            regex = runtime_re.search(new_line)
                                            runtime += int(regex.group(2))
                                
                                    iter.append( (e_par, runtime) )
                                    
                            iterF.append(iter)
                            
                    # RUN THE TRUE VERSION 
                    iterT = []
                    conf[name] = 1
                    path = compute_execution_path(conf) + 'Exp'
                    print(path)
                    ...
                    # We now have two lists, of the form: 
                    # List of (List of (e_par, runtime))
                    # For each dataset in the entire program. 
                    # Each inner list contains the information about a dataset's run through all 10 + 1 runs of the benchmark. 
                    # First, we remove this additional one (which is a warm-up run)
                    iterF = [iter[len(iter) / 11:] for iter in iterF]
                    iterT = [iter[len(iter) / 11:] for iter in iterT]

                    # Then, we convert each inner list to the mean of each datapoint for each value of E_par.
                    iterT = [ [ (iter[j][0] , np.mean([ iter[i][1] for i in range(j, len(iter))[::len(iter) // 10] ])) for j in range(len(iter) // 10)] for iter in iterT]
                    iterF = [ [ (iter[j][0] , np.mean([ iter[i][1] for i in range(j, len(iter))[::len(iter) // 10] ])) for j in range(len(iter) // 10)] for iter in iterF]

                    # Since these come from different threshold configurations, they might have differing lengths.
                    # I.e, one list might have E_par of 100, but another might not. This has to be combined, by extrapolating the data.
                    # If the other list has E_par = 50 and E_par = 150, we can make a point that is E_par = 100 with runtime of the E_par = 150.
                    Eset = set()
                    for run in iterT + iterF: 
                        names = [x[0] for  x in run]
                        for nameE in names:
                            Eset.add(int(nameE))
                            
                    sorted_E = np.sort(list(Eset))

                    for i, run in enumerate(iterT):
                        if len(run) == 0:
                            continue
                        runnames = [int(x[0]) for x in run]
                        maxindex = np.argmax(runnames)
                        maxval = run[maxindex][1] 
                        for nameE in sorted_E:
                            if all(n != nameE for n in runnames):
                                iterT[i].append((str(nameE), maxval))

                        iterT[i].sort(key = lambda x: int(x[0]), reverse=True)
                    
                    for i, run in enumerate(iterF):
                        if len(run) == 0:
                            continue
                        runnames = [int(x[0]) for x in run]
                        maxindex = np.argmax(runnames)
                        ...
                        for nameE in sorted_E:
                            if all(n != nameE for n in runnames):
                                iterF[i].append((str(nameE), maxval))

                        iterF[i].sort(key = lambda x: int(x[0]), reverse=True)

                    # Now we have matching-length lists of E_par to Runtimes for each dataset. 
                    # We want the cumulative area under the two curves according to a breakpoint which is minimised.
                    res = []
                    runtimesT = [[int(x[1]) for x in iter] for iter in iterT]
                    runtimesF = [[int(x[1]) for x in iter] for iter in iterF]                

                    # False up to i, then True for the rest
                    for i in range(len(runtimesT[0]) + 1):
                        try:
                    ...
                    if np.argmin(res) - 1 < 0:
                        winner_E = np.amax(sorted_E) + 1
                    else:
                        winner_E = sorted_E[::-1][np.argmin(res) - 1]
        
                    best_threshold_value  = winner_E
                else:
...
for i, program in enumerate(programs):
    ...
    (time_taken, bench_cmd, num_executed) = script_results[i]
    call_program(bench_cmd.replace(' --exclude-case=notune ', ' --exclude-case=tune ') + ' --json=instr-{}.json'.format(program[:-4]))
