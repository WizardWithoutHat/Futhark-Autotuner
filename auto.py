...
def extract_thresholds_and_values(program):
    ...
    with tempfile.NamedTemporaryFile() as json_tmp:
        ...
        for dataset in datasets:
            thresholds[dataset] = defaultdict(set)
...
with tempfile.NamedTemporaryFile() as json_tmp:
    ...
    best_times = {}
    best_versions = {}
    all_version_times[version_to_string(baseVersion)] = {}
...
for depth, i in deepest_first_order:
    ...
    branch_versions = extract_versions(len(extract_names([branch_tree[i]])), branch_tree[i])
    ...
    for j, current_version in enumerate(branch_versions[::-1]):
        ...
        # Add those values to a dict, named configuration.
        ...
        all_version_times[version_to_string(version)] = {}
        ...
        with tempfile.NamedTemporaryFile() as json_tmp:
            ...
            if(total_time < best_branch_time):
                best_branch_time = total_time #This is in sum of percentages pr. dataset
                best_branch_version = current_version
...
for dataset, version in best_versions.items():
    print("Dataset {} chose version {}".format(dataset, version))
