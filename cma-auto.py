...
def evaluation_function(threshold_list):
    ...
    with tempfile.NamedTemporaryFile() as json_tmp:
        ...
        if total_time < best_time:
            #print("[{}s] Considered new best time: {}".format(int(time.time() - start), total_time))
            best_time = runtime
            global best_version
            best_version = bench_cmd
    return total_time
...
def futhark_autotune_program(program):
    ...
    global best_version
    global best_time
    best_version = [-1 for name in threshold_names]
    best_time = 999999999999999999999999
